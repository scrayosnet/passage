...
- **Use plaintext `http://` addresses** -- Passage's gRPC client is built without a TLS backend. An
  `https://` address does not fail loudly; it connects in plaintext to the TLS port and every call
  then fails. For transport encryption, terminate TLS next to the adapter with a sidecar or service
  mesh, and keep the adapter itself reachable over a trusted network.
