...
For more advanced routing (like load balancing), see [Player Fill Strategy](/adapters/target-strategy/#player-fill-adapter).
...
For region-based routing with custom logic, you'll need a gRPC adapter. See [Custom gRPC Adapters](/advanced/grpc-adapters/) for implementation details.
