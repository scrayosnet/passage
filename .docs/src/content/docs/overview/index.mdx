...
<CardGrid>
  ...
  <Card title="Trivial Horizontal Scaling" icon="up-caret">
    No shared state means no complex multi-proxy cluster setup. Put a load balancer in front of as many Passage instances as you need. Each one operates independently, with an idle footprint in the low double-digit MB range.
  </Card>
...
- Low double-digit MB of RAM at idle; size limits with headroom for connection bursts
